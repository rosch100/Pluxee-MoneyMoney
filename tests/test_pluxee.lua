...
local testDir = debug.getinfo(1, "S").source:match("^@(.*/)") or "./"
local cryptoStub = dofile(testDir .. "mm_crypto_stub.lua")
...
MM = {
  ...
  -- In-process stubs mirroring MoneyMoney ≥ 2.5.2 crypto (no openssl/tmp).
  sha256 = cryptoStub.sha256,
  hexToBin = cryptoStub.hexToBin,
  base64urlencode = cryptoStub.base64urlencode,
  random = cryptoStub.random,
}
